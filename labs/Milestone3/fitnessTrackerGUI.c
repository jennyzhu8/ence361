...
#define DISPLAY_PERIOD      5   // 50ms period = 20Hz
#define BUTTON_PERIOD       1   // 10ms period = 100Hz
#define ADC_PERIOD          2   // 20ms period = 50Hz
#define STEP_DETECT_PERIOD  1   // 10ms period = 100Hz
#define CONGRATS_PERIOD     5   // 50ms period = 20Hz
...
// Accelerometer step detection logic
static int32_t accAvg;
static int32_t accThresh;
static bool stepDetected;

// Accelerometer values
static int32_t accSum_x;
static int32_t accSum_y;
static int32_t accSum_z;
static int32_t accMean_x;
static int32_t accMean_y;
static int32_t accMean_z;
static int32_t accRef_x;
static int32_t accRef_y;
static int32_t accRef_z;

static uint8_t congratsTime;
static bool congratsStartFlag;
static bool congratsEndFlag;
...
void stepDetectTask(void);
void congratsTask(void);
...
buttonTask(void)
{
    ...
    if ((butStateLeft == PUSHED) && (displayState != 3)) {
        displayState++;
        if (displayState > 2) {
            displayState = 0;
        }
    }
    if ((butStateRight == PUSHED) && (displayState != 3)) {
        displayState--;
        if (displayState < 0) {
            displayState = 2;
        }
...
/*********************************************************
 * Manage the accelerometer step detection logic
 *********************************************************/
void
stepDetectTask(void)
{
    ...
    accSum_z = 0;
    for (i = 0; i < BUF_SIZE; i++) {
        accSum_x = accSum_x + readCircBuf (&g_xAccBuffer);
        accSum_y = accSum_y + readCircBuf (&g_yAccBuffer);
        accSum_z = accSum_z + readCircBuf (&g_zAccBuffer);
    }

    // Calculate the rounded mean of the values in each acceleration buffer
    accMean_x = (2 * accSum_x + BUF_SIZE) / 2 / BUF_SIZE;
    accMean_y = (2 * accSum_y + BUF_SIZE) / 2 / BUF_SIZE;
    accMean_z = (2 * accSum_z + BUF_SIZE) / 2 / BUF_SIZE;

    // Corrected for reference orientation
    uint32_t acc_x = accMean_x - accRef_x;
    uint32_t acc_y = accMean_y - accRef_y;
    uint32_t acc_z = accMean_z - accRef_z;

    // Calculate the norm acceleration value
    int32_t accNorm = sqrt((acc_x * acc_x) + (acc_y * acc_y) + (acc_z * acc_z));

    // Step detection logic
    if (((accNorm - accAvg) > accThresh) && (!stepDetected)) {
        stepDetected = 1;
        stepCount++;
    }
    else if (((accNorm - accAvg) < accThresh) && (stepDetected)) {
        stepDetected = 0;
    }

    // Incorporate new norm value into running average
    accAvg = (6 * accAvg + accNorm)/7;
}

/*********************************************************
 * Congratulates the user for reaching their step goal
 *********************************************************/
void
congratsTask(void) {
    if ((stepCount >= stepGoal) && !congratsStartFlag) {
        congratsStartFlag = 1;
        congratsTime = 0;
        displayState = 3;
    }
    else if ((congratsStartFlag) && (!congratsEndFlag)) {
        congratsTime++;
        if (congratsTime >= 75) {
            congratsEndFlag = 1;
            displayState = 0;
...
main(void)
{
    ...
    uint32_t stepDetectTaskTimer = 0;
    uint32_t congratsTaskTimer = 0;
    ...
    while (1)
    {
        ...
        if ((currentTime - stepDetectTaskTimer) >= STEP_DETECT_PERIOD) {
            stepDetectTask();
            stepDetectTaskTimer = currentTime;
        }

        if ((currentTime - congratsTaskTimer) >= CONGRATS_PERIOD) {
            congratsTask();
            congratsTaskTimer = currentTime;
        }
    }
}
